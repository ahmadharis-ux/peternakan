    ...
    <div class="card">
        <div class="card-body">
            <div class="row my-2">
                ...
                <div class="col-sm-9">Rp. {{number_format($data->pembayaran->sum('debit'))}} </div>
                <div class="col-sm-3">Sisa Pembayaran : </div>
                <div class="col-sm-9">Rp. {{number_format(($data->bobot * $data->harga) - $data->pembayaran->sum('debit'))}} </div>
                <div class="col-sm-3">Status : </div>
                <div class="col-sm-9">
                @if (($data->bobot * $data->harga) == $data->pembayaran->sum('debit') || ($data->bobot * $data->harga) == $data->pembayaran->sum('kredit') )
                    Lunas
                @else
                    Belum Lunas
                @endif
                ...
                <div class="col-sm-9">Rp. {{number_format($data->bobot * $data->harga)}} </div>
                <div class="col-sm-3">Telah Dibayar : </div>
                <div class="col-sm-9">Rp. {{number_format($data->pembayaran->sum('kredit'))}} </div>
                <div class="col-sm-3">Sisa Pembayaran : </div>
                <div class="col-sm-9">Rp. {{number_format(($data->bobot * $data->harga) - $data->pembayaran->sum('kredit'))}} </div>
                <div class="col-sm-3">Status : </div>
                <div class="col-sm-9">
                @if (($data->bobot * $data->harga) == $data->pembayaran->sum('debit') || ($data->bobot * $data->harga) == $data->pembayaran->sum('kredit') )
                    Lunas
                ...
                <div class="col-sm-9">Rp. {{number_format($data->pembayaran->sum('kredit'))}} </div>
                <div class="col-sm-3">Sisa Pembayaran : </div>
                <div class="col-sm-9">Rp. {{number_format(($data->qty * $data->harga) - $data->pembayaran->sum('kredit'))}} </div>
                <div class="col-sm-5">
                    <hr>
                </div>
                <div class="col-sm-7"></div>
                <div class="col-sm-3">Status : </div>
                <div class="col-sm-9">
                @if (($data->qty * $data->harga) == $data->pembayaran->sum('debit') || ($data->qty * $data->harga) == $data->pembayaran->sum('kredit') )
                    Lunas
                @else
                    Belum Lunas
                @endif
                ...
                <div class="col-sm-9">
                    <a href="/export-pdf/pembayaran/{{$data->id}}" class="btn btn-md btn-success mt-3 ">Export</a><br>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-2 ms-auto mt-2"><h6><span class="badge bg-success">{{date("d - F - Y",strtotime($date))}}</span></h6></div>
            </div>
            @if ($data->jurnal->name == 'Piutang')
                ...
                <div class="row">
                    ...
                    <div class="col-sm mt-3">
                        ...
                        <select type="text" name="rekening_id" class="form-select">
                            ...
                            <option value="{{$item->id}}">{{$item->name}} {{$item->norek}} {{$item->bank}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="row">    
