...
<section class="section dashboard">
        ...
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <h5>Data Produk <button class="btn btn-sm btn-primary mx-3" data-bs-toggle="modal" data-bs-target="#produk">Tambah</button></h5>
                     <!-- Modal -->
                        ...
                        <div class="modal-content">
                            <form action="/storepiutang" method="post" enctype="multipart/form-data">
                                ...
                                <div class="modal-body">
                                    <div class="row">
                                        <input type="hidden" name="kas_id" value="{{$data->id}}" class="form-control">
    ...
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <h5>Riwayat Pembayaran<button class="btn btn-sm btn-primary mx-3" data-bs-toggle="modal" data-bs-target="#pembayaran">Tambah</button></h5>
                     <!-- Modal -->
                                        ...
                                        <div class="col-sm-12 mb-3">
                                            ...
                                            <select name="rekening_id" id="" class="form-select">
                                                @foreach ($rekening as $item)
                                                    <option value="{{$item->id}}">{{$item->name}} {{$item->norek}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div class="section">
                                <table class="table">
                                    ...
                                    <tbody>
                                      ...
                                      <tr>
                                        <th colspan="3">Subtotal</th>
                                        <td>Rp. {{number_format($data->pembayaran->sum('debit'))}}</td>
                                      </tr>
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div class="col-sm-4">
                            <div class="row mt-3">
                                ...
                                <div class="col-sm justify-content-end">
                                    Rp, {{number_format($data->piutang->sum('total_harga') + $data->operasional->sum('nominal') )}}
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                ...
                                <div class="col-sm justify-content-end">
                                   Rp. {{number_format($data->pembayaran->sum('debit'))}}
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                ...
                                <div class="col-sm justify-content-end">
                                   Rp. {{number_format($data->piutang->sum('total_harga') + $data->operasional->sum('nominal') - $data->pembayaran->sum('debit'))}}
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                ...
                                <div class="col-sm justify-content-end">
                                    @if ($data->pembayaran->sum('debit') == $data->piutang->sum('total_harga') + $data->operasional->sum('nominal'))
                                    Lunas
                                    @elseif($data->pembayaran->sum('debit') < $data->piutang->sum('total_harga') + $data->operasional->sum('nominal'))
                                    Belum Lunas   
                                    @endif
                                </div>
                            </div>
